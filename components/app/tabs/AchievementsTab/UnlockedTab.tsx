...
export default function UnlockedTab() {
  return (
    <div>
      ...
      <div className="flex flex-col gap-2">
        {fakeData.map((data, index) => (
          <div
            key={`unlocked-${data.achievementName}-${index}`}
            className="w-full px-1 py-4 flex items-center relative bg-[#2c2c30]"
          >
            <div className={`absolute h-full bg-[#4d4d52] -left-[1px]`} style={{ width: `${data.percentage}%` }} />
            <MdCheck className="mr-6 text-lg z-10" />
            <Image
              src={data.iconUrl}
              width={48}
              height={48}
            ...
            <div className="ml-auto font-bold z-10">
